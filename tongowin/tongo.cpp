// tongo.cpp : アプリケーションのエントリ ポイントを定義します。
//
...
TCHAR *szTitle			= _T("Tongo");		// タイトル バーのテキスト
TCHAR *szWindowClass	= _T("TONGOWIN");	// メイン ウィンドウ クラス名
TCHAR *szWindowClassL	= _T("TONGOWINL");	// レイヤー ウィンドウ クラス名
HWND hLayerWnd;
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wc.hIcon         = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_TONGOWIN));
...
LRESULT CALLBACK LayerWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_ERASEBKGND:
		...
		int w,h,h2;
		w = (int)floor(((-fHeight * 2.5) + 8) + 0.5); // Warning fix *NuTTeR*
		h = -fHeight * 2 + 8;
		h2 = h + fHeight;
...
BOOL uploadFile(HWND hwnd, LPCTSTR fileName)
{
	const TCHAR* UPLOAD_SERVER	= _T("tongo.nu11er.ru");
	...
	buf << sBoundary;
	buf << sCrLf;
	buf << "content-disposition: form-data; name=\"imagedata\"; filename=\"Tongo.com\"";
	buf << sCrLf;
	...
	// Referer addition
	const TCHAR* rf = _T("Referer: http://tongo.nu11er.ru/\r\n");
	BOOL rfResult = HttpAddRequestHeaders(
		hRequest, rf, _tcslen(rf), 
		HTTP_ADDREQ_FLAG_ADD | HTTP_ADDREQ_FLAG_REPLACE);
	if (FALSE == rfResult) {
		MessageBox(hwnd, _T("Cannot set referer"),
			szTitle, MB_ICONERROR | MB_OK);
		return FALSE;
	}
	...
	// Custom header platform addition
	const TCHAR* xplat = _T("X-Tongo-Platform: Windows\r\n");
	BOOL xplatResult = HttpAddRequestHeaders(
		hRequest, xplat, _tcslen(xplat), 
		HTTP_ADDREQ_FLAG_ADD | HTTP_ADDREQ_FLAG_REPLACE);
	if (FALSE == xplatResult) {
		MessageBox(hwnd, _T("Cannot set platform header"),
			szTitle, MB_ICONERROR | MB_OK);
		return FALSE;
	}
